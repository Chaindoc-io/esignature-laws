---
layout: home
title: E-Signature Laws by Jurisdiction
---

A community-maintained reference for electronic signature laws across the world's major jurisdictions. Each guide is grounded in primary sources (statutes, regulations, court rulings) and lists signature levels, blockchain-anchoring admissibility, and notable judicial precedents.

> **Disclaimer:** This site is informational, not legal advice. Always consult licensed counsel for binding decisions.

## Jurisdictions

The first jurisdiction guides will be published shortly. Watch [the repo](https://github.com/Chaindoc-io/esignature-laws) for updates.

## Open Data

The full dataset is published as machine-readable JSON: [`data/laws.json`](https://github.com/Chaindoc-io/esignature-laws/blob/main/data/laws.json), validated by [`data/schema.json`](https://github.com/Chaindoc-io/esignature-laws/blob/main/data/schema.json).

## License

Content is licensed under [CC-BY 4.0](https://github.com/Chaindoc-io/esignature-laws/blob/main/LICENSE). Attribution: "Source: Chaindoc — esignature-laws (https://docs.chaindoc.io)".

---

Maintained by the [Chaindoc team](https://chaindoc.io).
